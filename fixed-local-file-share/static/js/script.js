...
class LocalFileShare {
    ...
    init() {
        ...
        // Connect to WebSocket server and setup listeners immediately
        // (don't wait for IP — it resolves in the background)
        this.connectSocket();
        this.setupEventListeners();
        this.displayServerIP();

        // Get local IP address in background — updates UI when ready
        this.getLocalIP();
    }
    
    // Get local IP address — read directly from the URL (instant, no WebRTC needed)
    getLocalIP() {
        const url = new URL(window.location.href);
        this.deviceIP = url.hostname;
    ...
    connectSocket() {
        ...
        this.socket.on('client_connected', (data) => {
            ...
            // Update My Device panel from server data (source of truth)
            const me = data.clients.find(c => c.sid === this.socket.id);
            if (me) {
                document.getElementById('device-name').textContent = me.name;
                if (me.ip && me.ip !== 'Hidden') {
                    document.getElementById('device-ip').textContent = me.ip;
                    this.deviceIP = me.ip;
                }
            }
